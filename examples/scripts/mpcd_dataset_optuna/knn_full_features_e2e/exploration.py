...
def explore(trial: optuna.Trial, percent_pairs=100, ts_steps=200):
    return exploration(trial, percent_pairs, ts_steps).n_ts
...
if __name__ == '__main__':    
    ...
    model_data_all = ModelData(training_file='../chembl237_pec50_fps_ds_train.txt', # position of data points in feature space
                        response_file='../chembl237_pec50_response_train.txt') # corresponding response values
    ...
    study = optuna.create_study(study_name=Path(__file__).resolve().parent.name, load_if_exists=True, direction="maximize", pruner=optuna.pruners.MedianPruner(n_startup_trials=3, n_warmup_steps=min_starting_points, n_min_trials=3), storage=storage)
    ...
    study.optimize(lambda trial: explore(trial, 1, 5), n_trials=100, n_jobs=1, show_progress_bar=True, callbacks=[StopWhenOptimizationGoalReached(goal=50), StopWhenTrialKeepBeingPrunedCallback()])

    logger.info(f"Best transistion states during optimisation = {study.best_value}")
    ...
    logger.info(f"Total transistion states during optimisation = {ktn.n_ts}")
